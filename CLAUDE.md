...
7. **Release flow with four gates** – The release runs in four gates. Each gate fires only on its own configured trigger phrase from `workbench.trigger_phrases`. Never auto-advance to the next gate. Full procedure → [docs/release/VERSIONIERUNG.md](./docs/release/VERSIONIERUNG.md), orchestrator-Variante mit Sub-Agents → [docs/templates/RELEASE_START.md](./docs/templates/RELEASE_START.md).
   1. **`release` trigger** – Determine the next version per the schema (Phase end → Minor/Major, otherwise Patch; DEV-only additions never bump the version on their own). Run `git diff --name-only v<prev>..HEAD`, group the changed files by area, and run the release-wide review per [docs/release/REVIEW_TEMPLATE.md](./docs/release/REVIEW_TEMPLATE.md). For multi-area diffs: start one read-only sub-agent per area in parallel. Present a consolidated findings report and wait.
